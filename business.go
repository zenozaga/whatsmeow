...
func (cli *Client) GetCollections(params types.BusinessCollectionParams) {
...
func parseCatalog(node *waBinary.Node) (*types.Catalog, error) {
	...
	if !ok {
		return nil, fmt.Errorf("product_catalog not found")
	}

	d := xml.NewDecoder(bytes.NewReader([]byte(product_catalog.XMLString())))
	d.Strict = false
	err := d.Decode(catalog)
...
func (cli *Client) getCollections(jid types.JID, params *types.BusinessCollectionParams) {

	queryNodes := []waBinary.Node{

		// Set the limit of collections to fetch
		{
			Tag:     "collection_limit",
			Content: strconv.Itoa(params.Limit),
		},

		// Set the limit of items per collection
		{
			Tag:     "item_limit",
			Content: strconv.Itoa(params.ItemLimit),
		},
	...
	fmt.Println("RESULT", result.XMLString())
